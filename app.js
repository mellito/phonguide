...
app.get('/info',(req,res)=>{
    const date = new Date()
    res.send(`<p>Phonebook has info for  ${person.length} people</p> <p>${date.toUTCString()}</p>`)
})
...
app.delete('/api/persons/:id',(req,res)=>{
    ...
    if(!findPerson){
        ...
        const index= person.indexOf(findPerson);
...
app.post('/api/persons',(req,res)=>{
    ...
    else if(person.find((perso)=>  perso.name.trim()===req.body.name.trim())){
